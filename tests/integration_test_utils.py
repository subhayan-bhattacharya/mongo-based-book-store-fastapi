...
def validate_book_response(
    response, book_data_with_request=None, inserted_book_data=None
):
    ...
    all_authors_inserted = list(
        set(
            itertools.chain.from_iterable(
                [book["genres"] for book in all_books_inserted]
            )
        )
    )
    assert sorted(response_data) == sorted(all_authors_inserted)
...
def validate_all_books_second_page_response(
    response, all_books_inserted, books_returned_in_prev_request
):
    response_data = response.json()
    ...
    if len(all_books_inserted) - len(books_in_prev_response) < 3:
        ...
        assert len(response_data["books"]) == len(all_books_inserted) - len(
            books_in_prev_response
        )
